document.addEventListener('DOMContentLoaded', () => {
    // --- URL DE LA API DE GOOGLE SCRIPT ---
    const googleScriptURL = "https://script.google.com/macros/s/AKfycbwqkpIrmwD4SDeOda5ttFAqM_MPrlnqX_Ij6l51iGH88313xNoYpI4lQzsNou20-1MY/exec";
    ...
    if (amountInput && fromCurrencySelect && toCurrencySelect && calculateBtn && swapBtn && refreshBtn && resultAmount && exchangeRateDisplay && lastUpdateDisplay && loadingOverlay) {
        let exchangeRates = {};

        async function fetchExchangeRates() {
            if (loadingOverlay) loadingOverlay.style.display = 'flex';
            try {
                const response = await fetch(`${googleScriptURL}?action=getExchangeRates`);
                ...
                if (data.status === 'success' && data.rates) {
                    ...
                    throw new Error(data.message || 'No se pudieron obtener las tasas desde el servidor.');
                }
            } catch (error) {
        ...
        function calculateConversion() {
            ...
            if (amount <= 0 || !exchangeRates[from] || !exchangeRates[to]) {
                resultAmount.textContent = new Intl.NumberFormat('es-CO', { style: 'currency', currency: to }).format(0);
